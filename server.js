...
app.get('/dashboard', requireCustomerAuth, (req, res) => {
  ...
  <script>
    ...
    async function saveCardEdit(){
      ...
      const res = await fetch('/dashboard/api/card/' + encodeURIComponent(editingCardId) + '/details', {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ businessName, destinationUrl })
      });
    ...
    async function applyToAll(e){
      ...
      const res = await fetch('/dashboard/api/apply-to-all', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(Object.fromEntries(f))});
      if(!res.ok){ alert('Invalid destination URL.'); return; }
      location.reload();
    }
    async function toggleCardStatus(cardId, status){
      await fetch('/dashboard/api/card/' + cardId + '/status', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({status})});
      location.reload();
    }
  </script>
...
app.get('/admin', requireAuth, (req, res) => {
  ...
  <script>
    ...
    async function saveAdminPrompts(){
      ...
      if(prompts.some(p => !p)){
        alert('All 5 prompts are required.');
        return;
      }
